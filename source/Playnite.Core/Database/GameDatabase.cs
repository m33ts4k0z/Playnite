...
namespace Playnite.Database
{
    ...
    public partial class GameDatabase : IGameDatabaseMain, IDisposable
    {
        ...
        /// <summary>
        /// Host-app synchronization context used to marshal database events to the
        /// main thread. Set by PlayniteApplication; null in tests and headless use.
        /// </summary>
        ...
        public void OpenDatabase()
        {
            ...
            {
                MainThreadContext.Send((_) => DatabaseOpened?.Invoke(this, null), null);
            }
            else
            {
        ...
        public string AddFile(string path, Guid parentId, bool isImage, CancellationToken cancelToken)
        {
            ...
            if (path.IsHttpUrl())
            {
                ...
                try
                {
                    ...
                    if (isImage)
                    {
                        var converted = ImageConverter.ConvertToCompatibleFormat(downPath, Path.Combine(targetDir, Path.GetFileNameWithoutExtension(fileName)));
            ...
            else
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (isImage)
                        {
                            var converted = ImageConverter.ConvertToCompatibleFormat(path, Path.Combine(targetDir, Path.GetFileNameWithoutExtension(fileName)));
                            if (converted.IsNullOrEmpty())
                            {
                                return null;
                            }
        ...
        internal object GetFileLock(string filePath)
        {
        ...
        internal void ReleaseFileLock(string filePath)
        {
        ...
        public HashSet<string> GetImportedExeFiles()
        {
            ...
            foreach (var game in Games.Where(a => a.IsInstalled && a.GameActions.HasItems()))
            {
                ...
                {
                    imported.Add(ExpandGameVariables(game, action.Path, true, null) + action.Arguments ?? string.Empty);
                }
            }
